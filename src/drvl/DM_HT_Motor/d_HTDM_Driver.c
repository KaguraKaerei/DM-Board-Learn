...
// 记录每个电机当前模式
static HT_Mode_e ht_mode[6] = { (HT_Mode_e)0 };
static DM_Mode_e dm_mode[6] = { (DM_Mode_e)0 };
...
void Motor_HT_Set_DQVolt(Motor_ID_e id, float dq_v)
{
    ...
    int hid = Get_HT_ID(id);
    if(hid < 1) return;
    ht_mode[hid - 1] = (HT_Mode_e)HT_MODE_DQ_VOLT;
    HAL_Delay(1);
...
void Motor_HT_Set_DQCurrent(Motor_ID_e id, float dq_i)
{
    ...
    int hid = Get_HT_ID(id);
    if(hid < 1) return;
    ht_mode[hid - 1] = (HT_Mode_e)HT_MODE_DQ_CURRENT;
    HAL_Delay(1);
...
void Motor_HT_Set_Position(Motor_ID_e id, float pos_rad)
{
    ...
    int hid = Get_HT_ID(id);
    if(hid < 1) return;
    ht_mode[hid - 1] = (HT_Mode_e)HT_MODE_POS;
    HAL_Delay(1);
...
void Motor_HT_Set_Velocity(Motor_ID_e id, float vel_rad_s)
{
    ...
    int hid = Get_HT_ID(id);
    if(hid < 1) return;
    ht_mode[hid - 1] = (HT_Mode_e)HT_MODE_VEL;
    HAL_Delay(1);
...
void Motor_HT_Set_Torque(Motor_ID_e id, float torque)
{
    ...
    int hid = Get_HT_ID(id);
    if(hid < 1) return;
    ht_mode[hid - 1] = (HT_Mode_e)HT_MODE_TORQUE;
    HAL_Delay(1);
...
void Motor_HT_Set_PosVel(Motor_ID_e id, float pos_rad, float vel_rad_s)
{
    ...
    int hid = Get_HT_ID(id);
    if(hid < 1) return;
    ht_mode[hid - 1] = (HT_Mode_e)HT_MODE_POS_VEL;
    HAL_Delay(1);
...
void Motor_HT_Set_PosVelAcc(Motor_ID_e id, float pos_rad, float vel_rad_s, float acc_rad_s2)
{
    ...
    int hid = Get_HT_ID(id);
    if(hid < 1) return;
    ht_mode[hid - 1] = (HT_Mode_e)HT_MODE_TRAPEZOID;
    HAL_Delay(1);
...
void Motor_HT_Set_MIT(Motor_ID_e id, float pos_rad, float vel_rad_s, float torque, float kp, float kd)
{
    ...
    int hid = Get_HT_ID(id);
    if(hid < 1) return;
    ht_mode[hid - 1] = (HT_Mode_e)HT_MODE_MIT;
    HAL_Delay(1);
...
void Motor_HT_Stop(Motor_ID_e id)
{
    ...
    int hid = Get_HT_ID(id);
    if(hid < 1) return;
    ht_mode[hid - 1] = (HT_Mode_e)HT_MODE_STOP;
    HAL_Delay(1);
...
void Motor_HT_Brake(Motor_ID_e id)
{
    ...
    int hid = Get_HT_ID(id);
    if(hid < 1) return;
    ht_mode[hid - 1] = (HT_Mode_e)HT_MODE_BRAKE;
    HAL_Delay(1);
...
void Motor_DM_Set_MIT(Motor_ID_e id, float pos_rad, float vel_rad_s, float torque, float kp, float kd)
{
    ...
    int idx = Get_DM_ID(id);
    if(idx < 0) return;

    dm_mode[idx] = (DM_Mode_e)DM_MODE_MIT;
...
void Motor_DM_Set_PosVel(Motor_ID_e id, float pos_rad, float vel_rad_s)
{
    ...
    dm_mode[idx] = (DM_Mode_e)DM_MODE_POS;
...
void Motor_DM_Set_Velocity(Motor_ID_e id, float vel_rad_s)
{
    ...
    dm_mode[idx] = (DM_Mode_e)DM_MODE_SPD;
...
void Motor_DM_Set_PosVelCur(Motor_ID_e id, float pos_rad, float vel_rad_s, float cur_amp)
{
    ...
    dm_mode[idx] = (DM_Mode_e)DM_MODE_PSI;
...
static void DM_Apply_Param(int dm_idx)
{
    mode_e mode = (mode_e)dm_motor[dm_idx].ctrl.mode;
